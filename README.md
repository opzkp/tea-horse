...
$$\textbf{v}_{odd} = (v_1, v_3, ..., v_{d-1}) \quad \textbf{v}_{even} = (v_0, v_2, ..., v_{d-2})$$
...
$$P = C + \sum_{i=1}^m[{a_i}]U_i = \langle\textbf{v}, \textbf{G} \rangle + [r]H + \sum_{i=1}^m[\langle \textbf{v}, \textbf{x}_\textbf{i} \rangle]U_i \quad \quad \quad (3)$$
...
$$C = \sum_{i=1}^m \beta^i \cdot C_i = \sum_{i=1}^m \langle \beta^i \cdot \textbf{p}_i, \textbf{G} \rangle + [\beta^i \cdot r_i]H \quad \quad \quad (4)$$
...
$$a_i \cdot b_i = c_i \quad \quad \quad (5)$$
...
$$\delta_R \overset{\$}{\leftarrow} \mathbb{Z}_p,\quad R \leftarrow \text{Commit}(r(X, 1)X^{3N-1}; \delta_R) $$
...
$$\delta_{T_{lo}} \overset{\$}{\leftarrow} \mathbb{Z}_p,\quad T_{lo} \leftarrow \text{Commit}(t_{lo}(X, y); \delta_{T_{lo}}) $$

$$\delta_{T_{hi}} \overset{\$}{\leftarrow} \mathbb{Z}_p,\quad T_{hi} \leftarrow \text{Commit}(t_{hi}(X, y); \delta_{T_{hi}}) $$
...
$$(e = r(z, 1), proof_a) \leftarrow \text{Open}_\text{IPA}(R, z, r(X, 1), \delta_R)$$

$$(f = r(z, y), proof_b) \leftarrow \text{Open}_\text{IPA}(R, yz, r(X, 1), \delta_R)$$

$$(t_1 = t_{lo}(z, y), proof_{t_{lo}}) \leftarrow \text{Open}_\text{IPA}(T_{lo}, z, t_{lo}(X, y), \delta_{T_{lo}})$$

$$(t_2 = t_{hi}(z, y), proof_{t_{hi}}) \leftarrow \text{Open}_\text{IPA}(T_{hi}, z, t_{hi}(X, y), \delta_{T_{hi}})$$

Verifier need to verify all the above openings, compute $s = s(z, y)$, $s'$ from $s$, $k = k(y)$, then check if
$$t_1 + t_2 \overset{?}{=} e(f + s') - k \quad \quad \quad (9)$$
...
$$\quad \quad + [\langle \textbf{r} + \beta \cdot \textbf{t}_\textbf{lo} + \beta^2 \cdot \textbf{t}_\textbf{hi}, \textbf{z} \rangle]U_1 + [\langle \textbf{r}, \textbf{zy} \rangle]U_2 \quad \quad \quad (10)$$
...
$$\delta_{T_{lo}^{(i)}} \overset{\$}{\leftarrow} \mathbb{Z}_p,\quad T_{lo}^{(i)} \leftarrow \text{Commit}(t_{lo}^{(i)}(X, y); \delta_{T_{lo}^{(i)}}) $$

$$\delta_{T_{hi}^{(i)}} \overset{\$}{\leftarrow} \mathbb{Z}_p,\quad T_{hi}^{(i)} \leftarrow \text{Commit}(t_{hi}^{(i)}(X, y); \delta_{T_{hi}^{(i)}}) $$
