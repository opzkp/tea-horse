...
where $M\_{q,i}$ denotes the element at the position of $(q, i)$ of matrix $\textbf{M}$. And let $$ k(Y) = \sum\_{q=1}^Q Y^q \cdot k\_q$$
...
$$\delta\_R^{(i)} \overset{\$}{\leftarrow} \mathbb{Z}\_p,\quad R^{(i)} \leftarrow \text{Commit}(r^{(i)}(X, 1)X^{3N-1}; \delta\_R^{(i)}) $$
...
$$r(X, 1) = \sum\_{i=1}^m \alpha^i \cdot r^{(i)}(X, 1) \quad \quad s(X, y)= \sum\_{i=1}^m \alpha^i \cdot s^{(i)}(X, y) $$
