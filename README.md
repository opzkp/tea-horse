...
We use $[m]$ to denote close range $[1, m]$. $[\cdot]\cdot$ denotes scalar multiplication for an implicit EC group $\mathbb{G}$ of order $p$. $\lang \cdot, \cdot \rang$ denotes the inner product operator, regardless of the types of the parameters: they could be either $\Z_p \times \Z_p \rarr \Z_p$, or $\Z_p \times \mathbb{G} \rarr \mathbb{G}$.
...
For EC group $\mathbb{G}$ of order $p$, let $\textbf{G} \in \mathbb{G}^d, H \in \mathbb{G}$ be random generators. We commit to a vector $\textbf{v} \in \Z_p^d$ as
$$\text{Commit}(\textbf{G}, H, \textbf{v}, r): C = \lang \textbf{v}, \textbf{G} \rang + [r]H$$
where $r \in \Z_p$ is a random blinder providing hiding.

We may ignore the public parameters $\textbf{G}$ and $H$ in the parameter list, and simply put
$$\text{Commit}(\textbf{v}, r): C = \lang \textbf{v}, \textbf{G} \rang + [r]H \tag{1}$$
$\textbf{G}$ and $H$ should be fixed for Bitcoin use.

To open the commitment $(C, r)$, check if the identity holds
$$\text{Open}(C, \textbf{v}, r): C \overset{?}{=} \lang \textbf{v}, \textbf{G} \rang + [r]H$$
...
$$a = p(x_0) = \sum_{i=0}^{d-1}v_i \cdot x_0^i = \lang \textbf{v}, \textbf{x}_\textbf{0}\rang$$

To open $C$ at $x_0$ to value $a$, we use another random generator $U \in \mathbb{G}$, such that:
$$P = C + [a]U = \lang\textbf{v}, \textbf{G} \rang + [r]H + [\lang \textbf{v}, \textbf{x}_\textbf{0} \rang]U \tag{2}$$
...
Given a challenge value $\gamma \in \Z_p$, define new vectors of half the size:
...
$$P' = \lang \textbf{v}', \textbf{G}' \rang + [r]H + [\lang \textbf{v}', \textbf{x}_\textbf{0}' \rang]U$$
$$= P + [\gamma](\lang \textbf{v}_{even}, \textbf{G}_{odd} \rang + [\lang \textbf{v}_{even}, \textbf{x}_{0_{odd}} \rang]U) + [\gamma^{-1}](\lang \textbf{v}_{odd}, \textbf{G}_{even} \rang + [\lang \textbf{v}_{odd}, \textbf{x}_{0_{even}} \rang]U) $$
$$= P + [\gamma]L + [\gamma^{-1}]R$$
where $L$ and $R$ are provided from Prover to Verifier before the latter generating the challenge value $\gamma$:
$$L = \lang \textbf{v}_{even}, \textbf{G}_{odd} \rang + [\lang \textbf{v}_{even}, \textbf{x}_{0_{odd}} \rang]U$$
$$R = \lang \textbf{v}_{odd}, \textbf{G}_{even} \rang + [\lang \textbf{v}_{odd}, \textbf{x}_{0_{even}} \rang]U$$
...
$$P = C + \sum_{i=1}^m[{a_i}]U_i = \lang\textbf{v}, \textbf{G} \rang + [r]H + \sum_{i=1}^m[\lang \textbf{v}, \textbf{x}_\textbf{i} \rang]U_i$$
...
$$L = \lang \textbf{v}_{even}, \textbf{G}_{odd} \rang + \sum_{i=1}^m[\lang \textbf{v}_{even}, \textbf{x}_{i_{odd}} \rang]U_i$$
$$R = \lang \textbf{v}_{odd}, \textbf{G}_{even} \rang + \sum_{i=1}^m[\lang \textbf{v}_{odd}, \textbf{x}_{i_{even}} \rang]U_i$$
